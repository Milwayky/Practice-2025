...
namespace task02tests
{
    public class StudentServiceTests
    {
        ...
        {
            var result = _service.GetStudentsWithMinAverageGrade(4.6).ToList();
            Assert.Equal(2, result.Count);
        ...
        [Fact]
        public void GetStudentsOrderedByName_ReturnsCorrectOrderOfStudentsNames()
        {
        ...
        [Fact]
        public void GroupStudentsByFaculty_ReturnsCorrectOrderOfFaculties()
        {
            var result = _service.GroupStudentsByFaculty();
            Assert.Equal(2, result.Count());
            Assert.True(result["ФИТ"].Count() == 2);
            Assert.True(result["Экономика"].Count() == 1);
        }
